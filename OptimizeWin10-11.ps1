# --- Script created by Rohit Kr. Mandal ---
# Please ensure to run this script as Administrator.
...
Write-Output "--- Script created by Rohit Kr. Mandal ---"  # Creator's name for recognition.
...
# Set Yellow (in ABGR format, 0x00FFFF00) as the accent color
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Accent" -Name "AccentColor" -Type DWord -Value 0x00FFFF00
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Accent" -Name "AccentColorInactive" -Type DWord -Value 0x00FFFF00
...
Set-ItemProperty -Path "HKCU:\Control Panel\Desktop" -Name "Wallpaper" -Type String -Value "$env:SystemRoot\Web\Wallpaper\Windows\Black.jpg"
...
# List of non-Windows Update-related services
$services = @(
    "WpnUserService_3c167", "Winmgmt", "FontCache", "AppIDSvc", "AppXSvc", 
    "ClipSVC", "StorSvc", "AxInstSV", "AJRouter", "ALG", "BthAvctpSvc",
    "BDESVC", "wbengine", "BTAGService", "bthserv",
    "BluetoothUserService_409c6", "CertPropSvc", "Browser", "DiagTrack",
    "VaultSvc", "dmwappushservice", "DevicePickerUserSvc_409c6",
    "diagsvc", "DPS", "WdiServiceHost", "WdiSystemHost", "DispBrokerDesktopSvc",
    "TrkWks", "MapsBroker", "Fax", "fhsvc", "fdPHost", "FDResPub",
    "BcastDVRUserService_409c6", "lfsvc", "HvHost", "vmickvpexchange",
    "vmicguestinterface", "vmicshutdown", "vmicheartbeat", "vmicvmsession",
    "vmicrdv", "vmictimesync", "vmicvss", "SharedAccess", "iphlpsvc",
    "diagnosticshub.standardcollector.service", "MicrosoftEdgeElevationService",
    "edgeupdate", "edgeupdatem", "swprv", "Netlogon", "defragsvc",
    "WpcMonSvc", "SEMgrSvc", "PhoneSvc", "Spooler", "PrintNotify",
    "RmSvc", "RasAuto", "RasMan", "SessionEnv", "TermService",
    "UmRdpService", "RpcLocator", "RemoteRegistry", "RetailDemo",
    "RemoteAccess", "seclogon", "SensorDataService", "SensrSvc", "SensorService",
    "LanmanServer", "SCardSvr", "ScDeviceEnum", "SCPolicySvc",
    "OneSyncSvc_409c6", "SysMain", "lmhosts", "TapiSrv", "Themes",
    "TabletInputService", "VSS", "WalletService", "SDRSVC", "WbioSrvc",
    "WerSvc", "Wecsvc", "stisvc", "wisvc", "WMPNetworkSvc", "icssvc",
    "WinRM", "WSearch", "W32Time", "dot3svc", "workfolderssvc",
    "WwanSvc", "LanmanWorkstation", "XboxGipSvc", "XblAuthManager",
    "XboxNetApiSvc", "XblGameSave", "p2psvc", "p2pimsvc", "AppXSvc"
)
...
Write-Output "All non-Windows Update services have been disabled."

# Ask the user if they want to enable or disable Windows Update
$choice = Read-Host "Do you want to enable Windows Update? (Y/N)"

# List of Windows Update-related services
$services = @(
    ...
    "InstallService"  # Windows Installer Service
)
...
Write-Output "--- Disabling app access to 'Notifications' ---"
$appPrivacyKeyPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\AppPrivacy'
...
# Set 'LetAppsAccessNotifications' to '2'
Set-ItemProperty -Path $appPrivacyKeyPath -Name 'LetAppsAccessNotifications' -Value 2 -Type DWord
Set-ItemProperty -Path $appPrivacyKeyPath -Name 'LetAppsAccessNotifications_UserInControlOfTheseApps' -Value $null -Type MultiString
Set-ItemProperty -Path $appPrivacyKeyPath -Name 'LetAppsAccessNotifications_ForceAllowTheseApps' -Value $null -Type MultiString
Set-ItemProperty -Path $appPrivacyKeyPath -Name 'LetAppsAccessNotifications_ForceDenyTheseApps' -Value $null -Type MultiString
Write-Output "Access to 'Notifications' settings has been modified through GPO."

# Disable notifications capability
$userNotificationListenerKeyPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\userNotificationListener'
$data = 'Deny'

If (-Not (Test-Path $userNotificationListenerKeyPath)) {
    New-Item -Path $userNotificationListenerKeyPath -ItemType Directory | Out-Null
}
Set-ItemProperty -Path $userNotificationListenerKeyPath -Name 'Value' -Value $data
Write-Output "Capability 'User Notification Listener' access has been set to 'Deny'."

# Disable notifications access in older Windows versions - {52079E78-A92B-413F-B213-E8FE35712E72}
Write-Output "--- Disabling app access to GUID {52079E78-A92B-413F-B213-E8FE35712E72} ---"
$guidKeyPath = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global\{52079E78-A92B-413F-B213-E8FE35712E72}'

If (-Not (Test-Path $guidKeyPath)) {
    New-Item -Path $guidKeyPath -ItemType Directory | Out-Null
}
Set-ItemProperty -Path $guidKeyPath -Name 'Value' -Value $data
Write-Output "Access to GUID {52079E78-A92B-413F-B213-E8FE35712E72} has been set to 'Deny'."
